...
const sliceAngle = Math.PI / 9; // Adjust the angle to control the size of the slice

function createCircularSlice(innerRadius, outerRadius, radialSegments, tubularSegments, startAngle, endAngle, isInnermostSlice , h) {
  ...
  slice.position.y = h  ; // Position the slice slightly above the grassland surface to avoid z-fighting
...
const numSlices = 20 ; // Adjust the number of slices as needed
...
for (let i = 0; i < numSlices; i++) {
  ...
  const circularSlice = createCircularSlice(innerRadius, outerRadius, radialSegments, tubularSegments, startAngle, endAngle, isInnermostSlice , i/2 + 1);
...
// Define the radius and angle for positioning the industries
const radius = 30; // Adjust the radius as needed
const angleStep = (Math.PI * 2) / numSlices; // Angle step between each industry

// Calculate the initial angle offset
const initialAngleOffset = Math.PI / 2; // Adjust the initial angle offset as needed

// Loop through the industries
for (let i = 0; i < numSlices; i++) {
  const angle = i * angleStep + initialAngleOffset; // Calculate the angle for the current industry

  // Calculate the position of the industry based on the angle and radius
  const x = radius * Math.cos(angle);
  const z = radius * Math.sin(angle);

  // Create the industry text mesh
  const textGeometry = new THREE.TextGeometry(industryNames[i], {
    font: font,
    size: 2,
    height: 0.05,
  });
  ...
  let tiltAngle = 0;
  textMesh.rotation.y = tiltAngle - angle;
